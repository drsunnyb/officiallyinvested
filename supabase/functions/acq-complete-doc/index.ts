...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const contextText = 'KNOWN DEAL CONTEXT:\n' + JSON.stringify({ deal: deal ? { name: deal.name, asset_type: deal.asset_type, sector: deal.sector, asking_price: deal.asking_price } : null, original_submission: submission, current_assessment: assessment, figures_on_file: facts, other_documents: otherDocs, engine: val?.result ?? null, notes_and_correspondence: comms }).slice(0, 90000);

...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const otherDocs = (await sql`select file_name, doc_summary, extracted_text from acq.documents where deal_id=${doc.deal_id} and id <> ${doc.id} order by uploaded_at desc limit 20`).map((d: any) => ({ file: d.file_name, summary: d.doc_summary, content: String(d.extracted_text || '').slice(0, 8000) }));
    const kb = (await sql`select file_name, summary, extracted_text from acq.knowledge_docs where org_id=${doc.org_id} and status='done' order by updated_at desc limit 25`).map((k: any) => ({ name: k.file_name, summary: k.summary, excerpt: String(k.extracted_text || '').slice(0, 4000) }));
    ...
    const system =
      ...
      `\n\nTASK: Produce a COMPLETED, filled-in version of the document below. Keep its structure and headings, and fill in AS MUCH AS YOU POSSIBLY CAN from the KNOWN DEAL CONTEXT and the DETAILS THE USER JUST SUPPLIED. Cross-reference the context by MEANING, not exact wording: portfolio value equals GDV for a development; asking price equals purchase price; day-one cash equals equity or deposit; working capital required equals the funding need. Use every relevant figure and fact the context contains. Where a required value is not stated but can be reasonably DERIVED by simple arithmetic from the worked-through economics (for example total development cost approx GDV minus target profit; build/construction budget approx total cost minus land and fees; funding requirement approx total cost minus equity or land already committed), COMPUTE it, show the one-line working, and label it indicative. Only where a value cannot be stated or derived should you write a short bracketed placeholder like [to be confirmed]. Never fabricate figures that are neither stated nor derivable. ` +
      `You are also given the FIRM KNOWLEDGE BASE (its frameworks, templates, standard terms and methodology); follow the firm's templates, house style and standard wording where relevant. ` +
    ...
    const contextText = 'KNOWN DEAL CONTEXT:\n' + JSON.stringify({ deal: deal ? { name: deal.name, asset_type: deal.asset_type, sector: deal.sector, asking_price: deal.asking_price } : null, original_submission: submission, current_assessment: assessment, figures_on_file: facts, other_documents: otherDocs, firm_knowledge_base: kb, engine: val?.result ?? null, notes_and_correspondence: comms }).slice(0, 110000);

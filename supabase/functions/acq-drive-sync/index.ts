...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!acc.root_folder_id) { await sql.end({ timeout: 5 }); return json({ error: 'Set your Deals folder ID in Settings first.' }, 400); }
    ...
    let created_folders = 0, ingested_docs = 0, new_deals = 0;
    const results: any[] = [];
    const root = acc.root_folder_id;
    ...
    return json({ ok: true, created_folders, ingested_docs, new_deals, results });
  } catch (e) {

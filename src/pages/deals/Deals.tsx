...
import {
  ...
  ChevronLeft, Sparkles, HelpCircle, X, CalendarClock, ThumbsDown, LogOut, User,
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4" onClick={onClose}>
      <div className={card + ' p-6 max-w-sm w-full'} onClick={(e) => e.stopPropagation()}>
        ...
        <button onClick={async () => { if (!supabase) return; const { error } = await supabase.auth.signInWithOAuth({ provider: 'google', options: { redirectTo: window.location.href } }); if (error) setMsg(error.message); }}
          className="w-full flex items-center justify-center gap-2.5 border border-gray-300 rounded-xl py-3 text-sm font-semibold text-gray-700 hover:bg-gray-50 transition mt-4">
        ...
        <div className="flex items-center gap-3 py-2"><div className="flex-1 h-px bg-gray-200" /><span className="text-[11px] text-gray-400 uppercase tracking-wider">or</span><div className="flex-1 h-px bg-gray-200" /></div>
